...
class TestCustomDispatcher(CacheStringSourceBaseTest):
    ...
    def test_z_dispatcher_closure(self):
        @jit(nopython=True, _target="Z")
        def incr(a):
            return a + 1
        ...
        @jit(nopython=True, _target="Z", cache=True)
        def foo(a):
            x, incr = d[0]
            y, decr = d[1]
            return a + incr(x) + decr(y)

        self.assertEqual(foo(1), 2)
        self.check_hits(foo, 0, 1)
        self.run_in_separate_process()
